...
def test_refresh_jti_one_time_use(monkeypatch):
    ...
    monkeypatch.setattr(
        "services.refresh_token_store.get_redis_or_none", lambda: redis
    )
    jti = new_refresh_jti()
...
def test_consume_refresh_jti_without_redis(monkeypatch):
    monkeypatch.setattr(
        "services.refresh_token_store.get_redis_or_none", lambda: None
    )
    assert consume_refresh_jti(1, "abc") is True
